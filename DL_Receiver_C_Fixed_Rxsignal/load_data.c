...
void load_rx_data_ant2(rxData_ant2_t *rx_data)
{
	...
	// RX DATA MATRIX
	sprintf(fname, "RxSignal2RxData/rxDataMat/rxDataMat_snr%u_ant1.txt", snr_test);
	read_file(fname, NUM_ITEM*2*SMPL_PER_PRMB, data_in);
	load_rx_mat3d(NUM_ITEM, 2, SMPL_PER_PRMB, rx_data->rxDataMat_1, data_in);

	sprintf(fname, "RxSignal2RxData/rxDataMat/rxDataMat_snr%u_ant2.txt", snr_test);
	read_file(fname, NUM_ITEM*2*SMPL_PER_PRMB, data_in);
